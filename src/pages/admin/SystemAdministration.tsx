...
const SystemAdministration = () => {
	...
	return (
		...
		>
			...
			{submitted && (
				<div className="mb-4 text-green-600">Action logged!</div>
			)}
			...
			<ul className="space-y-2">
				{logs.map((log, idx) => (
					<li key={idx} className="flex justify-between items-center border rounded p-2 bg-gray-50">
						<span>{log.action} <span className="text-xs text-gray-500">({log.date})</span></span>
						<Button asChild size="sm" variant="outline"><a href="/admin/system-configuration">Config</a></Button>
						<Button asChild size="sm" variant="outline"><a href="/admin/maintenance">Maintenance</a></Button>
					</li>
				))}
			</ul>
		</PageTemplate>
