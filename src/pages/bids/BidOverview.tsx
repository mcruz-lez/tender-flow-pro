...
const BidOverview = () => {
  ...
  const filteredBids = bids.filter(bid => 
    ...
    if (activeTab === "active") return ["Submitted", "Draft", "Under Review"].includes(bid.status);
  ...
  const stats = {
    ...
    winRate: Math.round((bids.filter(b => b.status === "Awarded").length / bids.filter(b => ["Awarded", "Rejected"].includes(b.status)).length) * 100) || 0
  };
        ...
        <div className="p-6">
          ...
          <Breadcrumb className="mb-6">
            <BreadcrumbList>
              ...
              <BreadcrumbItem>
                <BreadcrumbPage className="text-white">Bid Management</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
              ...
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-3xl font-bold text-white">{stats.awarded}</p>
                  </div>
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-6">
            <TabsList className="glass-card border-purple-500/20">
              <TabsTrigger value="all" className="data-[state=active]:bg-purple-600">All Bids</TabsTrigger>
              <TabsTrigger value="active" className="data-[state=active]:bg-blue-600">Active</TabsTrigger>
              <TabsTrigger value="draft" className="data-[state=active]:bg-yellow-600">Draft</TabsTrigger>
              ...
              {viewMode === "grid" ? (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {filteredBids.map((bid) => (
                    <Card key={bid.id} className="glass-card border-purple-500/20 hover:border-purple-400/40 transition-all duration-300 transform hover:scale-105 hover:shadow-xl hover:shadow-purple-500/20">
                      <CardHeader className="pb-3">
                        <div className="flex justify-between items-start mb-2">
                          <Badge className={`${getStatusColor(bid.status)} border`}>
                            {bid.status}
                          </Badge>
                          <span className="text-sm text-purple-300">ID: {bid.id}</span>
                        </div>
                        <CardTitle className="text-lg text-white line-clamp-2">{bid.tenderTitle}</CardTitle>
                        <CardDescription className="text-purple-200">{bid.category}</CardDescription>
                      </CardHeader>
                      <CardContent>
                        <div className="space-y-3">
                          ...
                          <div className="flex justify-between text-sm">
                            ...
                            <span className={`font-medium ${getWinProbabilityColor(bid.winProbability)}`}>
                              {bid.winProbability}%
                            </span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-purple-300">Deadline:</span>
                            <span className="font-medium text-white">{bid.deadline}</span>
                          </div>
                          <div className="flex justify-between text-sm">
                            <span className="text-purple-300">Response Time:</span>
                            <span className="font-medium text-white">{bid.responseTime}</span>
                          </div>
                        </div>
                        <div className="mt-6 pt-4 border-t border-purple-500/20 flex gap-2">
                          <Button asChild variant="outline" size="sm" className="flex-1 glass-button">
                          ...
                          {bid.status === "Draft" && (
                            <Button asChild size="sm" className="flex-1 glass-button-primary">
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow className="border-purple-500/20">
                        <TableHead className="text-purple-200">Bid ID</TableHead>
                        <TableHead className="text-purple-200">Tender Title</TableHead>
                        <TableHead className="text-purple-200">Status</TableHead>
                        <TableHead className="text-purple-200">Bid Amount</TableHead>
                        <TableHead className="text-purple-200">Win Probability</TableHead>
                        <TableHead className="text-purple-200">Deadline</TableHead>
                        <TableHead className="text-purple-200">Actions</TableHead>
                      </TableRow>
                    </TableHeader>
                    <TableBody>
                      {filteredBids.map((bid) => (
                        <TableRow key={bid.id} className="border-purple-500/20 hover:bg-purple-500/5">
                          <TableCell className="text-white font-medium">{bid.id}</TableCell>
                          <TableCell className="text-white max-w-xs truncate">{bid.tenderTitle}</TableCell>
                          <TableCell>
                            <Badge className={`${getStatusColor(bid.status)} border`}>
                              {bid.status}
                            </Badge>
                          </TableCell>
                          <TableCell className="text-white font-medium">{bid.bidAmount}</TableCell>
                          <TableCell className={`font-medium ${getWinProbabilityColor(bid.winProbability)}`}>
                            {bid.winProbability}%
                          </TableCell>
                          <TableCell className="text-white">{bid.deadline}</TableCell>
                          <TableCell>
                            <div className="flex gap-2">
                              ...
                              {bid.status === "Draft" && (
                                <Button asChild size="sm" className="glass-button-primary">
                                  <Link to={`/bids/submit/${bid.tenderId}`}>
          ...
          <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-6">
            <Card className="glass-card border-blue-500/20 hover:border-blue-400/40 transition-all duration-300 transform hover:scale-105">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">AI Bid Analysis</h3>
                <p className="text-purple-200 text-sm mb-4">Get AI-powered insights for your bids</p>
            ...
            <Card className="glass-card border-green-500/20 hover:border-green-400/40 transition-all duration-300 transform hover:scale-105">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">Bid Library</h3>
                <p className="text-purple-200 text-sm mb-4">Access templates and past bids</p>
                <Button asChild className="glass-button-primary w-full">
            ...
            <Card className="glass-card border-purple-500/20 hover:border-purple-400/40 transition-all duration-300 transform hover:scale-105">
              <CardContent className="p-6 text-center">
                ...
                <h3 className="text-lg font-semibold text-white mb-2">Win Score</h3>
                <p className="text-purple-200 text-sm mb-4">Calculate your winning probability</p>
                <Button asChild className="glass-button-primary w-full">
